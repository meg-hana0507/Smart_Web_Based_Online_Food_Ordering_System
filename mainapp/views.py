...
def update_order_status(request, id):
    ...
    order = get_object_or_404(Order, id=id, restaurant_id=restaurant_id)

    if request.method == 'POST':
        status = request.POST.get('status')

        print("Updating Order:", id, "to", status)  # DEBUG
...
#     foods = FoodItem.objects.filter(id__in=recommended_ids)
